...
from anycsp.src.model.layers import Val2Val_Layer, Cst2Val_Layer, Val2Cst_Layer, Policy
from anycsp.src.utils.config_utils import read_config, write_config


class ANYCSP(Module):

    def __init__(self, args, model_dir, config):
    ...
    def forward(
        ...
        for s in range(steps):
            ...
            if cur_opt < opt:
                opt = cur_opt
                data.best_assignment = assignment.reshape(-1, self.args.k).T.argmax(axis=0)
                if verbose:
                    print('o {}'.format(data.constraints['ext'].cst_neg_mask.int().sum().cpu().numpy() - opt.int().cpu().numpy()))
                if keep_time:
